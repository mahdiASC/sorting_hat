    ...
    loadFiles() {
        return new Promise((resolve, reject) => {
            ...
            if (file_2) {
                //custom file loaded
                //check for correct file extension
                if (re.exec(file_2.name)) {
                    this.loadNewData(file_2).then(resolve);
                }else if (re2.exec(file_2.name)){
                    this.loadDefaultData(file_2,true).then(resolve);
                }else {
                    reject(`File "${file_2.name}" is not a TSV or CSV`);
                }            
            }
        })
    }

    loadDefaultData(file,flag=false) {
        return new Promise((resolve, reject) => {
